...
| M9 | 🟡 **SHIPPED 2026-08-03 (Phase 3.31)** — `turns.context` records who was in the room, and **turns captured with company present are excluded from personal-knowledge extraction in code**, not by prompt. `provenance` on knowledge and life events records source, turn range, context and time, so a bad memory can be traced and unwound. Original note: **Provenance typing**: every memory records *who said it, when, in what context* — guards against role-collapse (Shruti's words becoming Ayush's facts) and contamination, both active research problems with known designs. | M |
...
| # | Item | Size |
|---|---|---|
| H1 | 🟡 **SHIPPED 2026-08-03 (Phase 3.32)** — ten capability switches (gear → Capabilities) enforced at the dispatcher, the one choke point every brain tool passes. A disabled capability never runs; a refusal names the switch and is **not** logged as a failure. Defaults preserve today's behaviour except Terminal, now off by default. 50-check corpus asserts every acting tool is gated. Original note: **Per-capability gates for brain tools** — the privacy panel can't restrict what the brain may touch; a tool-permission matrix (already flagged in ORBIT.md) is the missing half of "guardrails & privacy". | M |
